...
const Map = ({ height = 100, device }) => {
  ...
  //         console.log(newPositions,"coords")
  //       setDefaultCenter(coords);
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      data.forEach((ele, idx) => {
        ...
        if (coords) {
          if (!newPositions[ele.device_id]) newPositions[ele.device_id] = [];
          newPositions[ele.device_id].push(coords);
          // For single device -- set center to latest valid coordinates
      ...
      if (device_id !== "all" && updatedCenter) {
        ...
        setZoom(13);
      } else if (device_id === "all" && Object.keys(newPositions).length > 0) {
        // Optionally set center to average, bounds, or default center
        const allCoords = Object.values(newPositions).flat();
        // Example: center on the first found device
        setDefaultCenter(allCoords[0]);
        setZoom(8); // Example zoom for multiple/all devices
      }
    }
  } catch (error) {
    console.error("Error fetching map info:", error);
