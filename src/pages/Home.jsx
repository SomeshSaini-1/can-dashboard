...
"Catalyst_Level"
    ...
    //  const [Driver_data, setDriver_data] = useState([]);
        async function fetchDrivers() {
          const url = await fetch(`${apiurl}/get_all_data`, {
            method: "POST",
            body: JSON.stringify({ sensorKey: "EngineSpeed_rpm" }),
            headers: { "Content-Type": "application/json" }
          });
          const res = await url.json();
            console.log(res);
            // setDevice_data((prev) => [...prev, ...res]);
        }
        ...
        <section className='mb-6 flex gap-4 w'>
          ...
          <div className=' font-semibold flex items-center justify-between mb-6'>
            ...
            <span className='flex gap-2 border flex items-center rounded p-2'>
              <SlidersHorizontal className='h-5' sensor_data = {sensor1}/>
